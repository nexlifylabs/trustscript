...
class TrustScript_Pending_Queue_Page {
	...
	public static function render() {
		...
		if ( $process_notice ) {
			...
			<div class="trustscript-queue-processed-notice">
				...
				<?php elseif ( $failed > 0 ) : ?>
					...
					?>
				<?php else : ?>
					&#x2139;&#xFE0F; <?php esc_html_e( 'Queue processed — no new orders were sent (quota may still be limited).', 'trustscript' ); ?>
				<?php endif; ?>
			</div>
			<?php
		}
		...
		<div class="wrap trustscript-queue-wrap">
			...
			<div class="trustscript-queue-summary">
				...
				<button type="button"
					id="trustscript-queue-process-all"
			...
			<div class="trustscript-queue-status-info">
				...
				<?php if ( $ready_count > 0 ) : ?>
					<span class="trustscript-queue-status-ready">⚡ <strong><?php echo esc_html( $ready_count ); ?> ready now</strong> (will process on next site visit or hourly cron)</span>
				<?php endif; ?>
				<?php if ( $scheduled_count > 0 ) : ?>
					<span class="trustscript-queue-status-scheduled">⏰ <strong><?php echo esc_html( $scheduled_count ); ?> scheduled</strong> (waiting for delay to arrive)</span>
				<?php endif; ?>
			</div>
			<?php endif; ?>
			...
			<div class="trustscript-queue-tabs wp-clearfix">
				...
				$tabs = array(
					'pending'   => sprintf( 
						...
						__( 'Pending (%d)', 'trustscript' ), TrustScript_Queue::count_pending() ),
					'failed'    => sprintf( 
						/* translators: %d = number of failed items */
						__( 'Failed (%d)', 'trustscript' ), TrustScript_Queue::count_failed() ),
					'completed' => sprintf( 
						/* translators: %d = number of completed items */
						__( 'Completed (%d)', 'trustscript' ), TrustScript_Queue::count_completed() ),
			...
			<table class="widefat fixed striped wp-list-table trustscript-queue-table" id="trustscript-queue-table">
				...
				<?php else : ?>
					...
					<tr id="trustscript-queue-row-<?php echo esc_attr( $item['id'] ); ?>" data-row-id="<?php echo esc_attr( $item['id'] ); ?>">
					...
					<td><?php echo $item['last_attempt_at'] ? esc_html( $item['last_attempt_at'] ) : '<em class=\"trustscript-queue-placeholder-text\">' . esc_html__( 'Never', 'trustscript' ) . '</em>'; ?></td>
					<td class=\"trustscript-queue-retry-count\"><?php echo esc_html( $item['retry_count'] ); ?></td>
						...
						<td class="trustscript-queue-actions">
							...
							<button type="button"
								class="button button-small trustscript-queue-retry"
								data-id="<?php echo esc_attr( $item['id'] ); ?>"
								data-nonce="<?php echo esc_attr( wp_create_nonce( 'trustscript_admin' ) ); ?>">
								<?php esc_html_e( 'Retry', 'trustscript' ); ?>
							</button>
							<button type="button"
								class="button button-small button-link-delete trustscript-queue-clear"
								data-id="<?php echo esc_attr( $item['id'] ); ?>"
							...
							<?php else : ?>
						<em class="trustscript-queue-no-action"><?php esc_html_e( 'No actions available', 'trustscript' ); ?></em>
							<?php endif; ?>
						</td>
					</tr>
					<?php endforeach; ?>
	...
	public function handle_queue_retry() {
		...
		$nonce_verified = wp_verify_nonce( wp_unslash( $_REQUEST['nonce'] ?? '' ), 'trustscript_admin' );
